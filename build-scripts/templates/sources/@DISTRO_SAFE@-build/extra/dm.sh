...
# cp -rf ${BASEDIR}/usr/local/share/backgrounds/@DISTRO_SAFE@/GreenLeaf.jpg ${BASEDIR}/usr/local/share/PCDM/themes/default/background.jpg
# sed -i "" 's/${command} stop/# ${command} stop/g' ${BASEDIR}/usr/local/etc/rc.d/pcdm

# # Auto login for user @DISTRO_USER@.
# sed -i "" "s/ENABLE_AUTO_LOGIN=FALSE/ENABLE_AUTO_LOGIN=TRUE/g" ${BASEDIR}/usr/local/etc/pcdm.conf.dist
# sed -i "" "s/AUTO_LOGIN_USER=no-username/AUTO_LOGIN_USER=@DISTRO_USER@/g" ${BASEDIR}/usr/local/etc/pcdm.conf.dist
# sed -i "" "s/AUTO_LOGIN_PASSWORD=no-password/AUTO_LOGIN_PASSWORD=@DISTRO_USER@/g" ${BASEDIR}/usr/local/etc/pcdm.conf.dist
