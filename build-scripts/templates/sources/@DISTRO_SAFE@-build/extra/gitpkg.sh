...
# Installing pc-sysinstall and @DISTRO_SAFE@ installer
if [ ! -d ${BASEDIR}/pcbsd ]; then
  echo "Downloading pcbsd tools from GitHub"
#  git clone https://github.com/@DISTRO@/pcbsd.git ${BASEDIR}/pcbsd >/dev/null 2>&1
   fetch https://github.com/@DISTRO@/pcbsd/archive/master.zip -o ${BASEDIR}/master.zip >/dev/null 2>&1
...
if [ ! -d ${BASEDIR}/operator ]; then
  ...
  git clone https://github.com/@DISTRO@/operator.git ${BASEDIR}/operator >/dev/null 2>&1
fi
...
# installing update-station
if [ ! -d ${BASEDIR}/update-station ]; then
  echo "# Downloading uodate-station from GitHub #"
  git clone https://github.com/@DISTRO@/update-station.git ${BASEDIR}/update-station >/dev/null 2>&1
fi
