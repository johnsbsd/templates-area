...
for left_files in ports @DISTRO_SAFE@ pcbsd gbi ; do
    rm -Rf ${BASEDIR}/${left_files}
done

if [ -f ${BASEDIR}/usr/local/etc/repos/@DISTRO@.conf ]; then
    rm -f  ${BASEDIR}/usr/local/etc/repos/@DISTRO@.conf
fi
