...
# If the @DISTRO_MAJOR@_DEBUG environment variable is set, be verbose.
[ ! -z "${@DISTRO_MAJOR@_DEBUG:-}" ] && set -x
...
if [ -n "${1:-}" ]; then
    ...
    LOGFILE=$(mktemp -q /tmp/@DISTRO_SAFE@.XXXXXX)
    REMOVELOG=1
fi
...
@DISTRO_MAJOR@_CONF=${@DISTRO_MAJOR@_CONF:-./conf/@DISTRO_SAFE@.conf}

[ -f ${@DISTRO_MAJOR@_CONF} ] && . ${@DISTRO_MAJOR@_CONF}
...
report_error() {
    if [ ! -z ${@DISTRO_MAJOR@_ERROR_MAIL:-} ]; then
	cat ${LOGFILE} | \
	    mail -s "@DISTRO@ build error in ${TARGET} phase" \
	    ${@DISTRO_MAJOR@_ERROR_MAIL}
    fi
}
