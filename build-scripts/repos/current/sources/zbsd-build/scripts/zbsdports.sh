...
PKGFILE=${PKGFILE:-${LOCALDIR}/conf/${PACK_PROFILE}-zbsd};
...
awk '/^zbsd_deps/,/^"""/' ${LOCALDIR}/packages/${PACK_PROFILE} | grep -v '"""' | grep -v '#' > ${LOCALDIR}/packages/${PACK_PROFILE}-gdepends

# If exist an old .packages file removes it
if [ -f ${LOCALDIR}/conf/${PACK_PROFILE}-zbsd ] ; then
  rm -f ${LOCALDIR}/conf/${PACK_PROFILE}-zbsd
fi

# Reads depends file and search for packages entries in each file from depends
# list, then append all packages found in zbsd file
while read pkgs ; do
awk '/^packages/,/^"""/' ${LOCALDIR}/packages/zbsd.d/$pkgs  >> ${LOCALDIR}/conf/${PACK_PROFILE}-gpackage
done < ${LOCALDIR}/packages/${PACK_PROFILE}-gdepends

# Removes """ and # from temporary package file
cat ${LOCALDIR}/conf/${PACK_PROFILE}-gpackage | grep -v '"""' | grep -v '#' > ${LOCALDIR}/conf/${PACK_PROFILE}-zbsd
...
# Compiling zbsd ports
if [ -d ${BASEDIR}/ports ]; then
  rm -Rf ${BASEDIR}/ports
fi
#mkdir -p ${BASEDIR}/usr/ports

echo "# Downloading zbsd ports from GitHub #"
git clone https://github.com/zBSD/ports.git ${BASEDIR}/ports >/dev/null 2>&1
...
# Build and install zbsd ports in chroot 
