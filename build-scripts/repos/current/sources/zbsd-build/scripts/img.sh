...
#
# $zBSD$
# $Id: img.sh,v 1.6 monday 12/26/11 Ovidiu Exp $
...
echo "/dev/ufs/${ZBSD_LABEL} / ufs ro,noatime 1 1" > ${CLONEDIR}/etc/fstab
...
makefs -B little -o label=${ZBSD_LABEL} ${IMGPATH} ${CLONEDIR}
if [ $? -ne 0 ]; then
  echo "makefs failed"
  exit 1
fi
