...
#
# $zBSD$
# $Id: pkginstall.sh,v 2.01 Tuesday, October 21 2014 Ovidiu Exp $
...
for left_files in ports zbsd pcbsd gbi ; do
    rm -Rf ${BASEDIR}/${left_files}
done

if [ -f ${BASEDIR}/usr/local/etc/repos/zBSD.conf ]; then
    rm -f  ${BASEDIR}/usr/local/etc/repos/zBSD.conf
fi
