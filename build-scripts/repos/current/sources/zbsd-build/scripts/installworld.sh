...
if [ -z "${START_IP}" ]; then
    ...
    if [ $(expr $last_seq + $INCREMENT ) -le 254 ];then
        ...
        echo "Invalid jail IP. Please defile START_IP in zbsd.defaults.conf.
...
INCREMENT value in zbsd.defaults.conf to have an valid IP
and restart build after using clscripts/clean_${PACK_PROFILE}_${ARCH}."
    ...
    if [ -z "${IFALREADY}" ]; then
        ...
        echo "Jail IP error. Jail's IP is already taken. Please change
START_IP value in zbsd.defaults.conf to have an valid IP
and restart build after using clscripts/clean_${PACK_PROFILE}_${ARCH}."
        exit 1
    fi
fi
