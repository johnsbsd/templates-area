...
# If the ZBSD_DEBUG environment variable is set, be verbose.
[ ! -z "${ZBSD_DEBUG:-}" ] && set -x
...
if [ -n "${1:-}" ]; then
    ...
    LOGFILE=$(mktemp -q /tmp/zbsd.XXXXXX)
    REMOVELOG=1
fi

cd $CURDIR

. ./conf/zbsd.defaults.conf

ZBSD_CONF=${ZBSD_CONF:-./conf/zbsd.conf}

[ -f ${ZBSD_CONF} ] && . ${ZBSD_CONF}
...
report_error() {
    if [ ! -z ${ZBSD_ERROR_MAIL:-} ]; then
	cat ${LOGFILE} | \
	    mail -s "zBSD build error in ${TARGET} phase" \
	    ${ZBSD_ERROR_MAIL}
    fi
}
