...
zBSD=`ls ${BASEDIR}/usr/local/share/applications/ | grep -v libreoffice | grep -v kde4 | grep -v screensavers` 
for desktop in $zBSD; do
...
default_zbsd_rc_conf()
{
  cp  ${BASEDIR}/etc/rc.conf ${BASEDIR}/etc/rc.conf.zbsd
}
