...
# cp -rf ${BASEDIR}/usr/local/share/backgrounds/zbsd/GreenLeaf.jpg ${BASEDIR}/usr/local/share/PCDM/themes/default/background.jpg
# sed -i "" 's/${command} stop/# ${command} stop/g' ${BASEDIR}/usr/local/etc/rc.d/pcdm

# # Auto login for user zetabsd.
# sed -i "" "s/ENABLE_AUTO_LOGIN=FALSE/ENABLE_AUTO_LOGIN=TRUE/g" ${BASEDIR}/usr/local/etc/pcdm.conf.dist
# sed -i "" "s/AUTO_LOGIN_USER=no-username/AUTO_LOGIN_USER=zetabsd/g" ${BASEDIR}/usr/local/etc/pcdm.conf.dist
# sed -i "" "s/AUTO_LOGIN_PASSWORD=no-password/AUTO_LOGIN_PASSWORD=zetabsd/g" ${BASEDIR}/usr/local/etc/pcdm.conf.dist
