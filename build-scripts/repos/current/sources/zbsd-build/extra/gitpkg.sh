...
# Installing pc-sysinstall and zbsd installer
if [ ! -d ${BASEDIR}/pcbsd ]; then
  echo "Downloading pcbsd tools from GitHub"
#  git clone https://github.com/zBSD/pcbsd.git ${BASEDIR}/pcbsd >/dev/null 2>&1
   fetch https://github.com/zBSD/pcbsd/archive/master.zip -o ${BASEDIR}/master.zip >/dev/null 2>&1
...
if [ ! -d ${BASEDIR}/operator ]; then
  ...
  git clone https://github.com/zBSD/operator.git ${BASEDIR}/operator >/dev/null 2>&1
fi
...
if [ ! -d ${BASEDIR}/update-station ]; then
  ...
  git clone https://github.com/zBSD/update-station.git ${BASEDIR}/update-station >/dev/null 2>&1
fi
